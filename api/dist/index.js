...
const updateUserOnlineStatus = (playerId, isOnline) => {
...
const updateUserInGameStatus = (playerId, isInGame, roomCode) => {
    const stmt = db.prepare(`
...
const sendGameStateToViewers = (roomCode) => {
...
const processQuestion = async (roomCode, questionData) => {
...
io.on('connection', (socket) => {
    ...
    socket.on('friend:invite', (data) => {
        ...
        const inviter = inviterStmt.get(inviterId);
        if (!inviter)
            return;
