...
const getSeasonName = (year, month) => {
...
export const SeasonService = {
    getCurrentSeason() {
      ...
      LIMIT 1
    `);
        const season = stmt.get(year, month);
        return season || null;
    },
    getSeasonById(id) {
    ...
    createSeason(year, month) {
        const existing = db.prepare('SELECT * FROM seasons WHERE year = ? AND month = ?').get(year, month);
        if (existing) {
            return existing;
        }
    ...
    `);
        ...
        const newSeason = db.prepare('SELECT * FROM seasons WHERE id = ?').get(Number(result.lastInsertRowid));
        return newSeason;
    },
    initCurrentSeason() {
        const now = new Date();
        ...
        const lastSeason = db.prepare('SELECT * FROM seasons WHERE year = ? AND month = ?').get(lastYear, lastMonth);
        if (lastSeason && lastSeason.status === 'active') {
            SeasonService.freezeSeason(lastSeason.id);
        }
        if (lastSeason && lastSeason.status === 'frozen') {
            const existingRankings = db.prepare('SELECT COUNT(*) as count FROM season_rankings WHERE season_id = ?').get(lastSeason.id);
    ...
    `);
        const transaction = db.transaction((players) => {
            for (let i = 0; i < players.length; i++) {
                ...
                insertStmt.run(seasonId, player.player_id, player.nickname, player.avatar, i + 1, player.score, player.wins, player.games, winRate);
            }
        });
        transaction(rows);
        db.prepare("UPDATE seasons SET status = 'frozen' WHERE id = ?").run(seasonId);
    },
    updateSeasonScore(playerId, score, isWinner) {
        const currentSeason = SeasonService.getCurrentSeason();
        if (!currentSeason)
            return;
        const stmt = db.prepare(`
    ...
    getCurrentSeasonRankings(pagination) {
        const currentSeason = SeasonService.getCurrentSeason();
        if (!currentSeason) {
            return {
                items: [],
    ...
    getSeasonRankings(seasonId, pagination) {
        const { page = 1, pageSize = 20 } = pagination || {};
        const offset = (page - 1) * pageSize;
        const countStmt = db.prepare(`
      SELECT COUNT(*) as count 
      ...
      WHERE season_id = ?
    `);
        const total = countStmt.get(seasonId).count;
        const stmt = db.prepare(`
      ...
      LIMIT ? OFFSET ?
    `);
        const rows = stmt.all(seasonId, pageSize, offset);
