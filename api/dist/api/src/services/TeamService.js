...
const mapToTeam = (row, memberCount) => ({
    id: row.id,
    name: row.name,
...
export const TeamService = {
    createTeam(playerId, name, avatar, description) {
        ...
        const existingName = nameStmt.get(name);
        if (existingName) {
    ...
    `);
        ...
        const items = rows.map((row) => mapToTeam(row, row.member_count));
        return {
            items,
            total,
            page,
    ...
    `);
        const row = stmt.get(teamId, playerId);
    ...
    kickMember(teamId, playerId, operatorId) {
    ...
    `);
        const pendingMatch = pendingMatchStmt.get(team1Id, team1Id, team2Id, team2Id);
        if (pendingMatch) {
    ...
    startMatch(matchId, operatorId) {
      ...
      LIMIT ?
    `);
        const team1Players = team1PlayersStmt.all(match.team1_id, TEAM_MATCH_PLAYERS);
      ...
      LIMIT ?
    `);
        const team2Players = team2PlayersStmt.all(match.team2_id, TEAM_MATCH_PLAYERS);
    ...
    updateMatchScore(matchId, team1Score, team2Score, winnerId) {
        const updateStmt = db.prepare(`
      UPDATE team_matches
    ...
    updatePlayerMatchStats(matchId, playerId, score, correctCount) {
        const updateStmt = db.prepare(`
