...
const calculateRank = (playerId: string, column: string): number => {
...
export const UserService = {
  getOrCreateStats(playerId: string, nickname: string): UserStatsRow {
    const findStmt = db.prepare('SELECT * FROM user_stats WHERE player_id = ?');
    let stats = findStmt.get(playerId) as UserStatsRow | undefined;
    ...
    if (!stats) {
      ...
      stats = findStmt.get(playerId) as UserStatsRow;
  ...
  updateStatsAfterGame(
    ...
    data: {
      ...
      maxStreak: number;
      isWinner: boolean;
      categories: Record<Category, { correct: number; total: number; score: number; avgTime: number }>;
    }
  ): void {
    ...
    const newAvgResponseTime = Math.round(
      (stats.avg_response_time * stats.total_questions + avgResponseTime * totalQuestions) / newTotalQuestions
    );
  ...
  getStats(playerId: string): UserStats | null {
    const findStmt = db.prepare('SELECT * FROM user_stats WHERE player_id = ?');
    const stats = findStmt.get(playerId) as UserStatsRow | undefined;
  ...
  updateWeeklyMonthlyScores(): void {
    db.exec(`
      UPDATE user_stats
