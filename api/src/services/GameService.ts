...
import type { Question, PlayerResult, ScoreDetail, DanmuMessage } from '../../../shared/types.js';
import type {
...
export interface GameState {
  ...
  playerAnswers: Map<string, Map<number, { answer: number; responseTime: number }>>;
  firstCorrectPlayer: Map<number, string>;
  scores: Map<string, {
    total: number;
    streak: number;
    details: ScoreDetail[];
...
export const getGameState = (roomCode: string): GameState | undefined => {
...
export const createPlayerRecord = (data: CreatePlayerRecordData): number => {
  const stmt = db.prepare(`
...
export const createQuestionRecord = (data: CreateQuestionRecordData): number => {
  const stmt = db.prepare(`
...
export const createAnswerRecord = (data: CreateAnswerRecordData): number => {
  const stmt = db.prepare(`
...
export const updatePlayerRecord = (gameId: number, playerId: string, data: UpdatePlayerRecordData): void => {
  const stmt = db.prepare(`
...
export const updateUserStats = (playerId: string, nickname: string, data: UpdateUserStatsData): void => {
...
export const updateQuestionStats = (questionId: number, isCorrect: boolean): void => {
  const stmt = db.prepare(`
    UPDATE questions
...
export const startGame = (
  ...
  const playerAnswers = new Map<string, Map<number, { answer: number; responseTime: number }>>();
...
export const submitAnswer = (
  ...
  answer: number,
  responseTime: number
): { isFirstCorrect: boolean; score?: ScoreDetail } => {
...
export const revealAnswer = (
  ...
  standings: { id: string; nickname: string; avatar: string; score: number; streak: number; isReady: boolean; isOnline: boolean }[];
} | null => {
  const gameState = gameStates.get(roomCode);
  ...
  const playersFromRoom = (globalThis as any).rooms?.get(roomCode)?.players || [];
...
export const finishGame = (roomCode: string): {
  recordId: number;
  finalStandings: PlayerResult[];
} | null => {
  const gameState = gameStates.get(roomCode);
  if (!gameState || !gameState.gameRecordId) return null;
  ...
  const room = (globalThis as any).rooms?.get(roomCode);
  ...
  const finalStandings: PlayerResult[] = players
    .map((player: any) => {
    ...
    .map((p: PlayerResult, index: number) => ({ ...p, rank: index + 1 }));
...
export const getViewerCount = (roomCode: string): number => {
  const gameState = gameStates.get(roomCode);
