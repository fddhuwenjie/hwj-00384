...
import type { Category, DatabaseQuestion, PaginatedResult, PaginationParams } from '../types/index.js';
import type { Question, CreateQuestionRequest } from '../../../shared/types.js';

const router = Router();

const mapRowToQuestion = (row: any): Question => {
  return {
    id: row.id,
    text: row.text,
...
const mapRowToDatabaseQuestion = (row: any): DatabaseQuestion => {
  return {
    id: row.id,
    text: row.text,
    options: [row.option_a, row.option_b, row.option_c, row.option_d],
...
router.get('/stats', asyncHandler(async (req: Request, res: Response) => {
  ...
  const categoryStmt = db.prepare(`
    ...
    GROUP BY category
  `);
  const categoryStats = categoryStmt.all() as { category: string; count: number }[];
  ...
  const usageStmt = db.prepare(`
    ...
    FROM questions
  `);
  const { totalUsage, totalCorrect } = usageStmt.get() as { totalUsage: number; totalCorrect: number };
...
router.put('/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  const data = req.body as Partial<CreateQuestionRequest>;
...
router.post('/batch', asyncHandler(async (req: Request, res: Response) => {
  const questions = req.body as CreateQuestionRequest[];

  if (!Array.isArray(questions) || questions.length === 0) {
    throw createError('Invalid batch data', 400);
  }

  const insert = db.transaction((items: CreateQuestionRequest[]) => {
    const stmt = db.prepare(`
      INSERT INTO questions (text, option_a, option_b, option_c, option_d, correct_answer, difficulty, category, analysis)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
